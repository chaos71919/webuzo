...
do
	...
	case "$KEY" in
		--v3)
		wget -O new_install.sh http://files.webuzo.com/new_install.sh >> /dev/null 2>&1
		chmod 0755 new_install.sh
		./new_install.sh $*
		exit;
	esac
done
...
theos="$(echo $REL | egrep -i '(cent|Scie|Red|Ubuntu|AlmaLinux)' )"
...
if [ "$OS" = redhat  ] ; then
	yum -y install ca-certificates gcc gcc-c++ curl unzip apr make cronie sendmail libnsl gawk >> $LOG 2>&1
